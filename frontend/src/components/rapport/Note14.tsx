...
const Note14: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note14) {
        const note14Data = dsf.notes.note14;

        if (note14Data.headerInfo) {
          setHeaderInfo(note14Data.headerInfo);
        }

        if (note14Data.rows) {
          setRows(note14Data.rows);
        }

        if (note14Data.comment !== undefined) {
          setComment(note14Data.comment);
        }
      }
    } catch (error) {
