...
const Note5Smt: React.FC = () => {
  ...
  // Lignes du journal
  const [items, setItems] = useState<CreanceImpItem[]>([
    {
      id: 1,
      date: "",
      numFacture: "",
  ...
  const downloadPDF = async () => {
    if (reportRef.current) {
      ...
      pdf.save("note_5_journal_creances_impayees.pdf");
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-6 font-sans text-xs">
      ...
      <div className="max-w-[297mm] mx-auto mb-6 flex justify-between items-center bg-white p-5 rounded-xl shadow-lg">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
          ...
          NOTE 5 - Journal de suivi des créances impayées
        </h1>
      ...
      >
        ...
        <div className="p-4">
                  ...
                  <td className="border border-gray-600 p-2">
                    {isEditing ? (
                      <input
                        ...
                        onChange={(e) =>
                          updateItem(item.id, "nomClient", e.target.value)
                        }
                      />
                    ) : (
                      item.nomClient || ""
          ...
          <div className="mt-8 px-4 pb-6 text-[10px] italic text-gray-700 text-center">
            * Annexer en pièces jointes le journal de suivi des créances
            impayées selon le modèle ci-dessus
          </div>
        </div>
      </div>
    </div>
