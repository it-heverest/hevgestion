...
const Note26: React.FC = () => {
  ...
  const loadNoteData = async () => {
    ...
    try {
      setIsLoading(true);
      const noteData = await notesService.getNoteData(folderId, "26") as any;
      ...
      console.error("Error loading Note 26 data:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveNoteData = async () => {
    ...
    try {
      ...
      const success = await notesService.saveNoteData(folderId, "26", noteData as any);
      if (success) {
        alert("Données Note 26 sauvegardées avec succès");
        setIsEditing(false);
      }
    } catch (error) {
      console.error("Error saving Note 26 data:", error);
  ...
  const renderRow = (row: OtherChargeRow) => (
    <tr key={row.id}>
      ...
      <td className="border border-gray-400 p-1 text-right">
        {isEditing && !hasFormula(`note26.rows.${row.id}`) ? (
          <input
            type="number"
            value={row.yearN}
          ...
          <FormulaValue formulaKey={`note26.rows.${row.id}`} label={row.label}>
