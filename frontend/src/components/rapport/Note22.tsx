...
const Note22: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note22) {
        const note22Data = dsf.notes.note22;

        if (note22Data.headerInfo) {
          setHeaderInfo(note22Data.headerInfo);
        }

        if (note22Data.rows) {
          setRows(note22Data.rows);
        }

        if (note22Data.comment !== undefined) {
          setComment(note22Data.comment);
        }
      }
    } catch (error) {
