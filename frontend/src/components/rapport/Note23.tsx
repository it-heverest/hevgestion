...
const Note23: React.FC = () => {
  ...
  const loadNoteData = async () => {
    ...
    try {
      setIsLoading(true);
      const noteData = await notesService.getNoteData(folderId, "23") as any;
      ...
      console.error("Error loading Note 23 data:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveNoteData = async () => {
    ...
    try {
      ...
      const success = await notesService.saveNoteData(folderId, "23", noteData as any);
      if (success) {
        alert("Données Note 23 sauvegardées avec succès");
        setIsEditing(false);
      }
    } catch (error) {
      console.error("Error saving Note 23 data:", error);
  ...
  const [rows, setRows] = useState<TransportRow[]>([
    { id: "1", label: "Transports sur ventes", yearN: 0, yearN1: 0 },
    {
      id: "2",
    ...
    { id: "3", label: "Transport du personnel", yearN: 0, yearN1: 0 },
    { id: "4", label: "Transports de plis", yearN: 0, yearN1: 0 },
    { id: "5", label: "Autres transports", yearN: 0, yearN1: 0 },
  ]);
  ...
  const downloadPDF = async () => {
    if (reportRef.current) {
      ...
      setTimeout(async () => {
        ...
        pdf.save("note_23_transports.pdf");
        setIsEditing(wasEditing);
      }, 100);
    }
  };

  const renderRow = (row: TransportRow) => (
    <tr key={row.id}>
      <td className="border border-gray-400 p-1 pl-2">{row.label}</td>
      <td className="border border-gray-400 p-1 text-right">
        {isEditing && !hasFormula(`note23.rows.${row.id}`) ? (
          <input
            type="number"
            value={row.yearN}
          ...
          <FormulaValue formulaKey={`note23.rows.${row.id}`} label={row.label}>
