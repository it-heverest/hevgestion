...
const Note15A: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note15A) {
        const note15AData = dsf.notes.note15A;

        if (note15AData.headerInfo) {
          setHeaderInfo(note15AData.headerInfo);
        }

        if (note15AData.rows) {
          setRows(note15AData.rows);
        }

        if (note15AData.comment !== undefined) {
          setComment(note15AData.comment);
        }
      }
    } catch (error) {
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const notes = { note15A: note15AData };
