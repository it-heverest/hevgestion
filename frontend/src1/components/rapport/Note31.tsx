...
const Note31: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note31) {
        const note31Data = dsf.notes.note31;

        if (note31Data.headerInfo) {
          setHeaderInfo(note31Data.headerInfo);
        }

        if (note31Data.rows) {
          setRows(note31Data.rows);
        }

        if (note31Data.comment !== undefined) {
          setComment(note31Data.comment);
        }
      }
    } catch (error) {
