...
const Note25: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note25) {
        const note25Data = dsf.notes.note25;

        if (note25Data.headerInfo) {
          setHeaderInfo(note25Data.headerInfo);
        }

        if (note25Data.rows) {
          setRows(note25Data.rows);
        }

        if (note25Data.comment !== undefined) {
          setComment(note25Data.comment);
        }
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 font-sans text-xs text-black">
      ...
      >
        {/* Numéro de page */}
        <div className="text-center font-bold mb-2 text-lg">37</div>
