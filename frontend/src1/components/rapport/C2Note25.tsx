...
const C2Note25: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.c2note25) {
        const data = dsf.notes.c2note25;
        if (data.headerInfo) setHeaderInfo(data.headerInfo);
        if (data.adValoremRows) setAdValoremRows(data.adValoremRows);
        if (data.specificRows) setSpecificRows(data.specificRows);
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const notes = { c2note25: c2note25Data };
