...
const Note24: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note24) {
        const note24Data = dsf.notes.note24;

        if (note24Data.headerInfo) {
          setHeaderInfo(note24Data.headerInfo);
        }

        if (note24Data.rows) {
          setRows(note24Data.rows);
        }

        if (note24Data.comment !== undefined) {
          setComment(note24Data.comment);
        }
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 font-sans text-xs text-black">
      ...
      >
        {/* Numéro de page */}
        <div className="text-center font-bold mb-2 text-lg">36</div>
