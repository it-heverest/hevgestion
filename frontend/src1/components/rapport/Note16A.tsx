...
const Note16A: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note16A) {
        const note16AData = dsf.notes.note16A;

        if (note16AData.headerInfo) {
          setHeaderInfo(note16AData.headerInfo);
        }

        if (note16AData.rows) {
          setRows(note16AData.rows);
        }

        if (note16AData.comment !== undefined) {
          setComment(note16AData.comment);
        }
      }
    } catch (error) {
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const notes = { note16A: note16AData };
