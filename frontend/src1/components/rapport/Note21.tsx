...
const Note21: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note21) {
        const note21Data = dsf.notes.note21;

        if (note21Data.headerInfo) {
          setHeaderInfo(note21Data.headerInfo);
        }

        if (note21Data.rows) {
          setRows(note21Data.rows);
        }

        if (note21Data.comment !== undefined) {
          setComment(note21Data.comment);
        }
      }
    } catch (error) {
