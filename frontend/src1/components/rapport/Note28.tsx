...
const Note28: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note28) {
        const note28Data = dsf.notes.note28;

        if (note28Data.headerInfo) {
          setHeaderInfo(note28Data.headerInfo);
        }

        if (note28Data.rows) {
          setRows(note28Data.rows);
        }

        if (note28Data.comment !== undefined) {
          setComment(note28Data.comment);
        }
      }
    } catch (error) {
