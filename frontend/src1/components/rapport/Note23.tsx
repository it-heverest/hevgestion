...
const Note23: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note23) {
        const note23Data = dsf.notes.note23;

        if (note23Data.headerInfo) {
          setHeaderInfo(note23Data.headerInfo);
        }

        if (note23Data.rows) {
          setRows(note23Data.rows);
        }

        if (note23Data.comment !== undefined) {
          setComment(note23Data.comment);
        }
      }
    } catch (error) {
