...
const Note27B: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note27B) {
        const note27BData = dsf.notes.note27B;

        if (note27BData.headerInfo) {
          setHeaderInfo(note27BData.headerInfo);
        }

        if (note27BData.rows) {
          setRows(note27BData.rows);
        }

        if (note27BData.comment !== undefined) {
          setComment(note27BData.comment);
        }
      }
    } catch (error) {
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const notes = { note27B: note27BData };
