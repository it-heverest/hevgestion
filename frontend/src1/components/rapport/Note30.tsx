...
const Note30: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note30) {
        const note30Data = dsf.notes.note30;

        if (note30Data.headerInfo) {
          setHeaderInfo(note30Data.headerInfo);
        }

        if (note30Data.charges) {
          setCharges(note30Data.charges);
        }

        if (note30Data.produits) {
          setProduits(note30Data.produits);
        }

        if (note30Data.comment !== undefined) {
          setComment(note30Data.comment);
        }
      }
    } catch (error) {
