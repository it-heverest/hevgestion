...
const Note18: React.FC = () => {
  ...
  const loadDSFData = async () => {
    ...
    try {
      ...
      if (dsf.notes && dsf.notes.note18) {
        const note18Data = dsf.notes.note18;

        if (note18Data.headerInfo) {
          setHeaderInfo(note18Data.headerInfo);
        }

        if (note18Data.rows) {
          setRows(note18Data.rows);
        }

        if (note18Data.comment !== undefined) {
          setComment(note18Data.comment);
        }
      }
    } catch (error) {
